...
lives_ok { $dat = $obj->get_minishelf_data('yak1ex'); } 'get_minishelf_data';
is($dat->{tana}{account}, 'yak1ex', 'tana->account');
is_deeply($dat->{category}, {}, 'no category');
is(@{$dat->{books}}, 5, 'books');
my %id = map { $_->{id} => 1 } @{$dat->{books}};

lives_ok { $dat = $obj->get_minishelf_data('yak1ex', rank => 5); } 'get_minishelf_data with rank';
is($dat->{tana}{account}, 'yak1ex', 'tana->account');
is_deeply($dat->{category}, {}, 'no category');
cmp_ok(@{$dat->{books}}, '>', 0, 'books');
isnt((grep { $id{$_} } map { $_->{id} } @{$dat->{books}}), 5, 'difference check');

lives_ok { $dat = $obj->get_minishelf_data('yak1ex', category => 2275669); } 'get_minishelf_data with category';
is($dat->{tana}{account}, 'yak1ex', 'tana->account');
